...
- Run un conteneur Keycloak pour le développement local :
  ```bash
  docker run -p 127.0.0.1:8095:8080 -e KC_BOOTSTRAP_ADMIN_USERNAME=admin -e KC_BOOTSTRAP_ADMIN_PASSWORD=admin -d --name gestion-stock-keycloak-local quay.io/keycloak/keycloak:26.4.7 start-dev
  ```
